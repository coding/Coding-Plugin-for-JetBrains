...
public class CodingNetShowCommitInBrowserAction extends DumbAwareAction {
  public CodingNetShowCommitInBrowserAction() {
    super("Open on GitHub", "Open selected commit in browser", CodingNetIcons.Github_icon);
  }
  ...
  protected static void openInBrowser(@NotNull Project project, @NotNull GitRepository repository, @NotNull String revisionHash) {
    String url = CodingNetUtil.findCodingNetRemoteUrl(repository);
    if (url == null) {
      CodingNetUtil.LOG.info(String.format("Repository is not under GitHub. Root: %s, Remotes: %s", repository.getRoot(),
                                        GitUtil.getPrintableRemotes(repository.getRemotes())));
      return;
    }
    CodingNetFullPath userAndRepository = CodingNetUrlUtil.getUserAndRepositoryFromRemoteUrl(url);
    if (userAndRepository == null) {
      CodingNetNotifications
        .showError(project, CodingNetOpenInBrowserAction.CANNOT_OPEN_IN_BROWSER, "Can't extract info about repository: " + url);
      return;
    }

    String githubUrl = CodingNetUrlUtil.getCodingHost() + '/' + userAndRepository.getUser() + '/'
  ...
  private static CommitData getDataFromHistory(AnActionEvent e) {
    ...
    if (repository == null || !CodingNetUtil.isRepositoryOnCoding(repository)) return null;
