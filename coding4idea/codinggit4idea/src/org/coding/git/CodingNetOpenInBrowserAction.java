...
import static org.coding.git.util.CodingNetUtil.LOG;

/**
 * @author robin
 */
public class CodingNetOpenInBrowserAction extends DumbAwareAction {
  public static final String CANNOT_OPEN_IN_BROWSER = "Can't open in browser";

  protected CodingNetOpenInBrowserAction() {
    super("Open on GitHub", "Open selected file in browser", CodingNetIcons.Github_icon);
  }
  ...
  public void update(AnActionEvent e) {
    ...
    if (!CodingNetUtil.isRepositoryOnCoding(gitRepository)) {
      e.getPresentation().setEnabledAndVisible(false);
      return;
    }
  ...
  public void actionPerformed(AnActionEvent e) {
    ...
    String githubRemoteUrl = CodingNetUtil.findCodingNetRemoteUrl(repository);
    if (githubRemoteUrl == null) {
      CodingNetNotifications.showError(project, CANNOT_OPEN_IN_BROWSER, "Can't find github remote");
      return;
    }

    String relativePath = VfsUtilCore.getRelativePath(virtualFile, repository.getRoot());
    if (relativePath == null) {
      CodingNetNotifications.showError(project, CANNOT_OPEN_IN_BROWSER, "File is not under repository root",
    ...
    if (hash == null) {
      CodingNetNotifications.showError(project, CANNOT_OPEN_IN_BROWSER, "Can't get last revision.");
      return;
    }
  ...
  private static String makeUrlToOpen(@Nullable Editor editor,
    ...
    String githubRepoUrl = CodingNetUrlUtil.makeCodingNetRepoUrlFromRemoteUrl(githubRemoteUrl);
